...
class ModelTrainer(object):
    ...
    def run_train(self):
        ...
        if dist_utils.is_local_master():
            ...
            logger.info(f'Updates per epoch (/gradient accumulation) = {updates_per_epoch}')
            logger.info(f'Steps per epoch (dataloader) = {len(train_dataloader)}')
            logger.info(f'Gradient accumulation steps = {args.gradient_accumulation_steps}')
            logger.info(f'Start offset of the epoch {self.start_epoch} (dataset) = step {self.start_offset}')
            logger.info(f'Updated step of the epoch {self.start_epoch} (dataloader) = step {updated_steps}')
            logger.info(f'Total remaining updates = {remaining_updates}')
    ...
    def _train_epoch(self, epoch, train_dataloader):
        ...
        for step, batch in enumerate(epoch_iterator, start=step_offset):
            ...
            if step % args.gradient_accumulation_steps != 0 and args.local_rank != -1:
            ...
            if (step - step_offset) % args.gradient_accumulation_steps == 0:
                ...
                self.optimizer.step()  # Optimizer step first
                self.scheduler.step()  # Then scheduler step
        ...
        if dist_utils.is_local_master():
            logger.info(f'Train: global step = {self.global_step}; step = {step}')
            logger.info(f'Avg. total Loss of epoch {epoch} = {epoch_loss:.3f}')
            logger.info("** ** * Saving fine-tuned model ** ** *")
            output_model_file = os.path.join(args.output_dir, f"model.{epoch}.bin")
